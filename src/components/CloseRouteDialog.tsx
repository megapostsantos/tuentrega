...
import { Camera, Loader2, X, AlertTriangle, CheckCircle2 } from "lucide-react";
...
const MOTIVO_INFO: Record<Motivo, { label: string; emoji: string; photoRequired: boolean; cls: string }> = {
  address_not_visited: { label: "Endereço não visitado", emoji: "🔴", photoRequired: true, cls: "border-red-300 hover:bg-red-50" },
  lost: { label: "Pacote perdido", emoji: "🔴", photoRequired: true, cls: "border-red-300 hover:bg-red-50" },
  damaged: { label: "Pacote danificado", emoji: "🟠", photoRequired: true, cls: "border-orange-300 hover:bg-orange-50" },
  could_not_deliver: { label: "Não foi possível entregar", emoji: "⚪", photoRequired: false, cls: "border-muted hover:bg-muted/50" },
};
...
export function CloseRouteDialog({ oferta, onClose, onClosed }: {
  ...
  function goStep3(occList = occ, deliveredN = delivered) {
    ...
    for (const o of occList) {
      if (o.motivo) impact += SCORE[o.motivo];
    }
  ...
  async function confirmClose() {
    ...
    try {
      ...
      // Create entrega record for payment
      ...
      <DialogContent className="max-h-[95vh] overflow-y-auto sm:max-w-lg">
        ...
        {step === 2 && (
          <Step2
            ...
            onBack={() => setStep(1)}
            onNext={() => {
              // validate all have motivo + required photos
              for (const o of occ) {
...
function Step2({
  ...
  function selectMotivo(idx: number, m: Motivo) {
    if (m === "address_not_visited") {
      setConfirmDanger({ idx, motivo: m });
      return;
    }
...
function CameraCapture({ ofertaId, packageNumber, onCancel, onCapture }: {
  ...
  async function snap() {
    ...
    for (let i = 0; i < 6; i++) {
      b = await new Promise<Blob | null>((res) => c.toBlob(res, "image/jpeg", q));
      if (!b || b.size <= 1_000_000) break;
      q -= 0.12;
    }
    if (!b) return toast.error("Falha ao capturar foto");
    setBlob(b);
  ...
  async function upload() {
    ...
    const { error } = await supabase.storage.from("entregas-fotos").upload(path, blob, {
  ...
  return (
    <Dialog open onOpenChange={onCancel}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader><DialogTitle>Foto do pacote {packageNumber}</DialogTitle></DialogHeader>
        <div className="space-y-3">
          {!preview ? (
            <>
              <video ref={videoRef} className="w-full rounded-md bg-black" playsInline muted />
              <Button className="w-full bg-primary text-primary-foreground" onClick={snap}>
            ...
            <>
              ...
              <div className="flex gap-2">
                <Button variant="outline" className="flex-1" onClick={() => { setPreview(null); setBlob(null); }}>Tirar de novo</Button>
                <Button className="flex-1 bg-primary text-primary-foreground" onClick={upload} disabled={uploading}>
                  {uploading ? <Loader2 className="h-4 w-4 animate-spin" /> : "Usar esta foto"}
                </Button>
              </div>
            </>
          )}
