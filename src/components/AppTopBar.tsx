  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes",
        ...
        (payload) => {
          setNotifs((prev) => {
            if (payload.eventType === "INSERT") return [payload.new as Notification, ...prev].slice(0, 50);
            if (payload.eventType === "UPDATE") return prev.map((n) => n.id === (payload.new as Notification).id ? (payload.new as Notification) : n);
    ...
    <header className="sticky top-0 z-30 h-[60px] border-b border-border bg-background/95 backdrop-blur elev-1">
      <div className="mx-auto flex h-full max-w-6xl items-center gap-2 px-3 sm:px-4">
        <button
          ...
          className="flex h-10 w-10 items-center justify-center rounded-full hover:bg-muted press-scale"
        >
        ...
        <div className="flex flex-1 items-center justify-center">
          <Link to="/dashboard" className="flex items-center gap-2">
            <span className="text-base font-black tracking-tight text-foreground sm:text-lg">BAG Envios</span>
          </Link>
        </div>
                  ...
                  {notifs.map((n) => (
                    <li key={n.id}>
                      <button
                        ...
                        className={`flex w-full items-start gap-3 px-5 py-4 text-left transition-colors hover:bg-muted/60 ${!n.lida ? "bg-primary/5" : ""}`}
                      >
                        <span className={`mt-1.5 h-2 w-2 shrink-0 rounded-full ${!n.lida ? "bg-primary" : "bg-transparent"}`} />
                        <div className="min-w-0 flex-1">
                          <div className="flex items-baseline justify-between gap-2">
                            <p className={`truncate text-sm ${!n.lida ? "font-semibold" : "font-medium"} text-foreground`}>{n.titulo}</p>
                        ...
                        {n.lida && <Check className="mt-1 h-3.5 w-3.5 shrink-0 text-muted-foreground/50" />}
                      </button>
                    </li>
                  ))}
                </ul>
